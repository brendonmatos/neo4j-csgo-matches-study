...
class InsertCSGO:
    ...
    def insert_player_country(self, player, country):
        self.session.execute_write(self._create_player_country, player, country)

    @staticmethod
    def _create_player_country(tx, player, country):
        tx.run("MATCH (b:Country {name: $country}) "
                        "MERGE (a:Player {name: $player}) "
                        "MERGE (a)-[r:FROM]->(b) "
                        "RETURN type(r)", player=player, country=country)

    def insert_country(self, country):
        self.session.execute_write(self._create_country, country)

    @staticmethod
    def _create_country(tx, country):
        tx.run("MERGE (a:Country {name: $country}) "
                        "RETURN a", country=country)

    def insert_match(self, match):
        self.session.execute_write(self._insert_match, match)

    @staticmethod
    def _insert_match(tx, match):
        tx.run("MERGE (a:Match {id: $match}) RETURN a", match=match)
    

    def insert_team(self, team):
        self.session.execute_write(self._insert_team, team)

    @staticmethod
    def _insert_team(tx, team):
        tx.run("MERGE (a:Team {name: $team}) RETURN a", team=team)

    def insert_player_match(self, player, match):
        self.session.execute_write(self._insert_player_match, player, match)
    
    @staticmethod
    def _insert_player_match(tx, player, match):
        tx.run("MATCH (a:Player {name: $player}) "
                        "MATCH (b:Match {id: $match}) "
                        "MERGE (a)-[r:PLAYER]->(b) "
                        "RETURN type(r)", player=player, match=match)
    
    def insert_team_match(self, team, match):
        self.session.execute_write(self._insert_team_match, team, match)
    ...
    def insert_winner_match(self, team, match):
        self.session.execute_write(self._insert_winner_match, team, match)
