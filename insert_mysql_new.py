...
class InsertCSGO:
    ...
    def insert_player_country(self, player, country):
        ...
        try:
            ...
            # Inserir o relacionamento entre jogador e país
            # Verificar se o relacionamento entre jogador e país já existe
            query = "SELECT COUNT(*) FROM player_country WHERE player_id = %s AND country_id = %s"
            cursor.execute(query, (player_id, country_id))
            result = cursor.fetchone()
            if result[0] == 0:
                # O relacionamento não existe, então podemos inserir
                query = "INSERT INTO player_country (player_id, country_id) VALUES (%s, %s)"
                cursor.execute(query, (player_id, country_id))
